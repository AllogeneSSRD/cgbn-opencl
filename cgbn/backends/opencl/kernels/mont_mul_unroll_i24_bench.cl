...
// ulong body = Level 1; u32 body = Level 2 (32-bit CIOS MAC).
...
__kernel void ecm_mont_mul_unroll_i24_u32_bench(__global const uint *a,
                                                  __global const uint *b,
                                                  __constant uint *n,
                                                  __global uint *out,
                                                  __constant uint *np0_ptr,
    ...
    for (uint it = 0u; it < iterations; ++it) {
        if (it == 0u) {
            mont_mul_unroll_i24_u32_body(out, a, b, n, base, np0);
        } else {
            mont_mul_unroll_i24_u32_body(out, out, b, n, base, np0);
        }
    }
}

__kernel void ecm_mont_sqr_unroll_i24_u32_bench(__global const uint *a,
                                                  __constant uint *n,
                                                  __global uint *out,
                                                  __constant uint *np0_ptr,
                                                  uint limbs, uint iterations) {
    ...
    for (uint it = 0u; it < iterations; ++it) {
        if (it == 0u) {
            mont_sqr_unroll_i24_u32_body(out, a, n, base, np0);
        } else {
            mont_sqr_unroll_i24_u32_body(out, out, n, base, np0);
        }
    }
}
