// Stage1 special_mult — 512-bit unroll-only (R = 2^32).
// Unrolled single-limb CIOS with 16 hardcoded limbs.
// Inlined into ecm_stage1.cl when MAX_LIMBS <= 16.
static inline void special_mult_ui32_unroll_512b(uint *r, uint m, const uint *N, uint np0, uint limbs) {
    uint t[17u];
    #pragma unroll
    for (uint i = 0u; i < 17u; ++i) t[i] = 0u;
    ...
    // 3) branchless conditional subtraction (same pattern as mont_mul_unroll_512b)
    {
        ...
        uint need_sub = (t[16u] != 0u) ? 1u : (borrow == 0ul ? 1u : 0u);
