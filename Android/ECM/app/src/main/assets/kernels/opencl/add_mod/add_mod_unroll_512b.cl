// Stage1 add_mod / unroll_512b (16 limbs): self-contained full unroll over MAX_LIMBS.
static inline void add_mod_unroll_512b_body(uint *r, const uint *a, const uint *b, const uint *N) {
    ulong carry_add = 0ul;
    ulong carry_sub = 1ul;
    #pragma unroll 16
...
static inline void add_mod_unroll_512b(uint *r, const uint *a, const uint *b, const uint *N, uint limbs) {
    if (limbs == MAX_LIMBS) { add_mod_unroll_512b_body(r, a, b, N); }
    (void)limbs;
}
