...
def emit_add_block(n: int) -> str:
    ...
    return f"""
// {n}-limb fused add-mod block; streaming __global (4096-bit unroll).
static inline void asm_fused_block{n}(__global const uint *a, __global const uint *b,
...
def emit_sub_block(n: int) -> str:
    ...
    return f"""
// {n}-limb fused sub-mod block; streaming __global (4096-bit unroll).
static inline void asm_sub_fused_block{n}(__global const uint *a, __global const uint *b,
...
def write_sub_block_file(out: Path, n: int, generator_name: str) -> None:
    # c_fix_add_n{N} lives in mp_addmod_asm_block{N}_generated.cl (included first).
    parts = [
        f"// AUTO-GENERATED by tools/{generator_name} — do not edit.\n",
        "#if defined(MP_ADDMOD_ASM_ENABLE) && defined(__AMDGCN__)\n\n",
        emit_sub_block(n),
